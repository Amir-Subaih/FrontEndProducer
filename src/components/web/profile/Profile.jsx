...
export default function Profile() {
  ...
  let [userId, setUserId] = useState(() => {
    ...
    console.log("this error is hear1");
  });
  ...
  useEffect(() => {
    if(userId){
      ...
      console.log("this error is hear2");
    }
  }, [userId]);
  ...
  const deleteAccount = async () => {
    if (userToken) {
      const { data } = await axios.delete(
        `https://backendproduce.onrender.com/api/users/${userId}`,
  ...
  return (
    <aside className={`${style.profile}`} dir='rtl'>
      ...
      <nav>
        ...
        <Link to="myOrders">طلباتي</Link>

        <Link  onClick={() => setShowModal(true)}>حذف الحساب</Link> 
      </nav>
