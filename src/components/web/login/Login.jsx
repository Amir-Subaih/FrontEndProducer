...
export default function Login() {
    ...
    const onSubmit = async users => {
           try {const { data } = await axios.post("https://backendproduce.onrender.com/api/auth/login", users);
            console.log(data);
            if (data.message === 'success') {
                    ...
                    setUserData(data.other); // Set userData here

                    if(data.other.isAdmin){
                        toast.success(`أهلا وسهلا ${data.other.name}`); 
                        navigate('/admin');
                    }else{
